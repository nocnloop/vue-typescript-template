...
@Component({ name: 'MenuEdit' })
export default class MenuEdit extends Vue {
  @Prop({ type: Object, default: () => null }) rowData!: any
  ...
  created() {
    this.nodeName = this.rowData.name
    this.formData = this.$copy(this.rowData)
  }
  ...
  save() {
    ;(this.$refs.form as Form).validate(async valid => {
      if (!valid) return
      try {
        await this.$confirm('确认提交吗?', '确认提示')
        this.isQuesting = true
        await menuAddOrEdit(this.formData, { successTip: true, errorTip: true })
        this.$emit('afterEdit')
      } catch (error) {
        console.log(error)
      } finally {
